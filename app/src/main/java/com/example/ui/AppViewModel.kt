...
class AppViewModel(application: Application, private val repository: LocalRepository) : AndroidViewModel(application) {
    ...
    val defaultScreens = listOf(
        Screen.DEEPA_AI, Screen.SEARCH, Screen.TASKS, Screen.CALENDAR, Screen.TIMER, Screen.HABITS, Screen.COUNTDOWN, Screen.JOURNAL, Screen.CONTACTS, Screen.FILE_EXPLORER, Screen.FINANCES, Screen.ANALYTICS, Screen.SETTINGS
    )
            ...
            try {
                ...
                val result = com.example.util.GoogleCalendarSyncHelper.syncGoogleCalendar(
                    ...
                    onUpdateTaskDescription = { task, updatedDesc ->
                        repository.updateTask(task.copy(description = updatedDesc))
                    }
                )
    ...
    fun queryLifeOS(query: String): String {
        ...
        if (tMatches.isEmpty() && hMatches.isEmpty() && jMatches.isEmpty() && cMatches.isEmpty()) {
